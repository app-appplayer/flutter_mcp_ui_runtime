...
class ToolActionExecutor extends ActionExecutor {
  ...
  Future<ActionResult> execute(
    ...
    if (action.containsKey('args')) {
      throw ArgumentError('Use "params" instead of deprecated "args" for tool actions');
    }
    ...
    try {
      ...
      if (_toolExecutors?[tool] == null && _toolExecutors?['default'] != null) {
        _logger.debug('Calling default tool executor with tool=$tool, params=$resolvedParams');
        result = await toolExecutor(tool, resolvedParams);
        _logger.debug('Default tool executor returned: $result');
      } else {
        _logger.debug('Calling specific tool executor with params=$resolvedParams');
...
class NavigationActionExecutor extends ActionExecutor {
  static bool Function(String action, String route, Map<String, dynamic> params)? _globalNavigationHandler;
  ...
  static void setGlobalNavigationHandler(bool Function(String action, String route, Map<String, dynamic> params)? handler) {
    MCPLogger('NavigationActionExecutor').info('Setting global navigation handler: ${handler != null}');
    _globalNavigationHandler = handler;
    MCPLogger('NavigationActionExecutor').info('Global handler is now: ${_globalNavigationHandler != null}');
  }
  ...
  Future<ActionResult> execute(
    ...
    if (route == null && ['push', 'replace'].contains(actionType)) {
      return ActionResult.error('Route is required for $actionType navigation action');
    }
    ...
    MCPLogger('NavigationActionExecutor').debug('Context handler: ${handler != null}');
    if (handler == null && context.renderer.navigationHandler != null) {
      handler = context.renderer.navigationHandler;
      MCPLogger('NavigationActionExecutor').debug('Using renderer handler: ${handler != null}');
    }
    if (handler == null) {
      handler = _globalNavigationHandler;
      MCPLogger('NavigationActionExecutor').debug('Using global handler: ${handler != null}');
    }
    ...
    MCPLogger('NavigationActionExecutor').debug('Getting navigatorKey from NavigationService');
    MCPLogger('NavigationActionExecutor').debug('NavigatorKey: $navigatorKey');
    MCPLogger('NavigationActionExecutor').debug('NavigatorKey hashCode: ${navigatorKey.hashCode}');
    
    final navigatorState = navigatorKey.currentState;
    MCPLogger('NavigationActionExecutor').debug('Navigator currentState: $navigatorState');
    
    if (navigatorState == null) {
      MCPLogger('NavigationActionExecutor').debug('Navigator state is null - navigation not possible');
      MCPLogger('NavigationActionExecutor').debug('NavigationService instance: ${NavigationService.instance}');
      MCPLogger('NavigationActionExecutor').debug('NavigationService navigatorKey: ${NavigationService.instance.navigatorKey}');
      MCPLogger('NavigationActionExecutor').debug('NavigationService navigatorKey hashCode: ${NavigationService.instance.navigatorKey.hashCode}');
    ...
    try {
      ...
      MCPLogger('NavigationActionExecutor').debug('Navigation completed: $actionType to $route');
      return ActionResult.success();
    } catch (e) {
    ...
    try {
      switch (actionType) {
        ...
        case 'remove':
          ...
          if (index != null) {
            // Index-based removal
            final indexNum = index is int ? index : int.tryParse(index.toString());
            if (indexNum != null) {
              final current = context.getValue(binding) as List? ?? [];
  ...
  Future<ActionResult> execute(
    ...
    if (method != null && target != null) {
      ...
      if (context.resourceHandler != null) {
        ...
        final result = await context.resourceHandler!(resourceName ?? 'default', method, target, data);
          ...
          final dialogActions = actions?.map((actionDef) {
            ...
            return DialogAction(
              ...
              onPressed: () async {
                ...
                final navigatorContext = DialogService.navigatorKey.currentContext;
