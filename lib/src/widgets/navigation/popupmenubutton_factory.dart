...
class PopupMenuButtonWidgetFactory extends WidgetFactory {
  ...
  Widget build(Map<String, dynamic> definition, RenderContext context) {
    ...
    final padding = parseEdgeInsets(properties['padding']) ?? const EdgeInsets.all(8.0);
    ...
    final items = itemsData.map((item) => _buildPopupMenuItem(item, context)).toList();
    ...
    if (childrenDef != null && childrenDef.isNotEmpty) {
      child = context.renderer.renderWidget(childrenDef.first as Map<String, dynamic>, context);
    } else {
    ...
    Widget popupMenuButton = PopupMenuButton<String>(
      ...
      onSelected: onSelected != null ? (value) {
  ...
  PopupMenuItem<String> _buildPopupMenuItem(dynamic itemData, RenderContext context) {
    if (itemData is Map<String, dynamic>) {
      ...
      final child = itemData['child'] != null
          ? context.renderer.renderWidget(itemData['child'] as Map<String, dynamic>, context)
