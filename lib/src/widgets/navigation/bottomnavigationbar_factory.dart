...
class BottomNavigationBarWidgetFactory extends WidgetFactory {
  ...
  Widget build(Map<String, dynamic> definition, RenderContext context) {
    ...
    // Extract properties
    final currentIndex = context.resolve<int>(properties['currentIndex']) as int? ?? 0;
    ...
    final selectedItemColor = parseColor(context.resolve(properties['selectedItemColor']));
    final unselectedItemColor = parseColor(context.resolve(properties['unselectedItemColor']));
    ...
    final unselectedFontSize = properties['unselectedFontSize']?.toDouble() ?? 12.0;
    final selectedLabelStyle = _parseTextStyle(properties['selectedLabelStyle'], context);
    ...
    final items = itemsData.map<BottomNavigationBarItem>((item) {
      if (item is Map<String, dynamic>) {
        return BottomNavigationBarItem(
          ...
          activeIcon: item['activeIcon'] != null ? _buildIcon(item['activeIcon'], context) : null,
  ...
  IconThemeData? _parseIconThemeData(Map<String, dynamic>? data, RenderContext context) {
