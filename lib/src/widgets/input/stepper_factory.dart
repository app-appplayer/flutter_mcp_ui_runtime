...
class StepperWidgetFactory extends WidgetFactory {
  ...
  Widget build(Map<String, dynamic> definition, RenderContext context) {
    ...
    final stepperType = _parseStepperType(properties['type']) ?? StepperType.vertical;
    ...
    final steps = stepsData.map((stepData) => _buildStep(stepData, context)).toList();
    
    // Extract action handlers
    final onStepTapped = properties['onStepTapped'] as Map<String, dynamic>?;
    final onStepContinue = properties['onStepContinue'] as Map<String, dynamic>?;
    ...
    Widget stepper = Stepper(
      ...
      margin: margin,
      steps: steps,
      onStepTapped: onStepTapped != null ? (step) {
  ...
  Step _buildStep(dynamic stepData, RenderContext context) {
    if (stepData is Map<String, dynamic>) {
      final title = stepData['title'] != null 
          ...
          : Text(context.resolve<String>(stepData['titleText']) as String? ?? '');
