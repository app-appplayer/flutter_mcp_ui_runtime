  ...
  Widget build(Map<String, dynamic> definition, RenderContext context) {
    ...
    Widget rangePicker = InkWell(
      onTap: enabled ? () async {
        ...
        if (pickedRange != null) {
          final formattedStart = "${pickedRange.start.year}-${pickedRange.start.month.toString().padLeft(2, '0')}-${pickedRange.start.day.toString().padLeft(2, '0')}";
          final formattedEnd = "${pickedRange.end.year}-${pickedRange.end.month.toString().padLeft(2, '0')}-${pickedRange.end.day.toString().padLeft(2, '0')}";
