...
class ContainerWidgetFactory extends WidgetFactory {
  ...
  Widget build(Map<String, dynamic> definition, RenderContext context) {
    ...
    final padding = _resolveEdgeInsets(properties[core.PropertyKeys.padding], context);
    final margin = _resolveEdgeInsets(properties[core.PropertyKeys.margin], context);
    final width = context.resolve(properties[core.PropertyKeys.width])?.toDouble();
    ...
    if (properties[core.PropertyKeys.decoration] != null) {
      decoration = _parseDecoration(properties[core.PropertyKeys.decoration], context);
    } else if (directColor != null || directBorderRadius != null || directBorder != null || directBoxShadow != null) {
  ...
  BoxDecoration? _parseDecoration(dynamic decoration, RenderContext context) {
    ...
    if (decoration is Map<String, dynamic>) {
      return BoxDecoration(
        ...
        borderRadius: _parseBorderRadius(decoration[core.PropertyKeys.borderRadius]),
        border: _parseBorder(decoration[core.PropertyKeys.border], context),
        boxShadow: _parseBoxShadow(decoration[core.PropertyKeys.shadow], context),
      );
    }
  ...
  Border? _parseBorder(dynamic border, RenderContext context) {
    ...
    if (border is Map<String, dynamic>) {
      final color = parseColor(context.resolve(border[core.PropertyKeys.color])) ?? Colors.black;
  ...
  List<BoxShadow>? _parseBoxShadow(dynamic shadow, RenderContext context) {
    ...
    if (shadow is Map<String, dynamic>) {
      return [
        BoxShadow(
          color: parseColor(context.resolve(shadow[core.PropertyKeys.color])) ?? Colors.black,
          blurRadius: context.resolve(shadow['blur'] ?? shadow['blurRadius'])?.toDouble() ?? 0,
          spreadRadius: context.resolve(shadow['spread'] ?? shadow['spreadRadius'])?.toDouble() ?? 0,
    ...
    if (shadow is List) {
      return shadow.map((s) => BoxShadow(
        color: parseColor(context.resolve(s[core.PropertyKeys.color])) ?? Colors.black,
        blurRadius: context.resolve(s['blur'] ?? s['blurRadius'])?.toDouble() ?? 0,
        spreadRadius: context.resolve(s['spread'] ?? s['spreadRadius'])?.toDouble() ?? 0,
